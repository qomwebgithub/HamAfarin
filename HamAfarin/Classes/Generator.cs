...
namespace HamAfarin
{
    public class Generator
    {
        ...
        public static string InvoiceNumber()
        {
            ...
            var invoiceNumber = DateTime.Now.ToString("yyyyMMddHHmmssffff");
            List<Tbl_BusinessPlanPayment> qlstBusinessPlanPayment = db.Tbl_BusinessPlanPayment.Where(p => p.IsDelete == false).ToList();

            while (qlstBusinessPlanPayment.Any(f => f.InvoiceNumber == invoiceNumber))
                invoiceNumber = DateTime.Now.ToString("yyyyMMddHHmmssffff");
